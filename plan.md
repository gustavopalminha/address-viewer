...
Goal: Establish a clean separation of concerns [52] and enforce code quality [54] from day one.

Repo Structure: Use a monorepo structure (e.g., /client and /server directories) for ease of delivery, but ensure they are logically completely separate applications to satisfy the architecture requirement [52].

Language: Use TypeScript for both the back-end and front-end. This demonstrates consistency [56] and strong command of one of their core required skills.

Tooling Standard: Set up ESLint and Prettier immediately to ensure consistent coding style throughout the codebase [55].
...
Data Ingestion: Load the adresses.json file [4].

Search Implementation:

Implement the recommended trie-search for rapid retrieval [9], or a similar efficient data structure if you prefer, to make the dataset searchable [8].
...
Limiting: Slice the output to return a maximum of 20 results [11].
